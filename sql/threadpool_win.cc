...
struct connection_t
{
  ...
  Vio *get_vio()
  {
    return thd->get_protocol_classic()->get_net()->vio;
  }
};
  ...
  PVOID context, PTP_WORK work)
{
  ...
  if (!connection->thd || login(connection, instance))
  {
    destroy_connection(connection, instance);
    free(connection);
  }
  ...
  PVOID parameter, PTP_TIMER timer)
{
  ...
  if (timeout <= now())
  {
    ...
    if (con->thd)
	     tp_post_kill_notification(con->thd);
  }
  ...
  PVOID context, PTP_WAIT wait,TP_WAIT_RESULT wait_result)
{
  ...
  */
  HANDLE h = con->get_vio()->event_server_wrote;
  SetEvent(h);
...
*/
bool tp_add_connection(Channel_info *ci)
{
  ...
  connection->channel_info= ci;

  PTP_WORK wrk =  CreateThreadpoolWork(login_callback,connection, NULL);
  if (!wrk)
  {
    free(connection);
    return true;
